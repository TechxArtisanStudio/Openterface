...
from config import (
    OUTPUT_DIR, REPORTS_DIR, SUPPORTED_LANGUAGES, BASE_URL
)
...
class ReportGenerator:
    ...
    def generate_summary_report(self, df: pd.DataFrame) -> str:
        ...
        if 'language' in df.columns:
            ...
            lang_stats['Success Rate %'] = (lang_stats['Valid URLs'] / lang_stats['Total URLs'] * 100).round(1)
            
            for lang, row in lang_stats.iterrows():
                lang_name = SUPPORTED_LANGUAGES.get(lang, lang)
                report_lines.append(f"- **{lang_name} ({lang})**: {row['Total URLs']} URLs, {row['Success Rate %']:.1f}% valid")
            report_lines.append("")
        ...
        if not response_times.empty:
            ...
            report_lines.append(f"- **95th Percentile**: {response_times.quantile(0.95):.3f}s")
            report_lines.append(f"- **Max**: {response_times.max():.3f}s")
            report_lines.append("")
        
        # Usage frequency analysis
        if 'usage_count' in df.columns:
            usage_stats = df.groupby('usage_count').size().sort_index(ascending=False).head(10)
            report_lines.append("## Most Frequently Used URLs")
            for usage_count, url_count in usage_stats.items():
                report_lines.append(f"- **Used {usage_count} times**: {url_count} unique URLs")
            report_lines.append("")
    ...
    def generate_broken_links_report(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        broken_links = broken_links.sort_values(['error_message', 'usage_count'], ascending=[True, False])
    ...
    def generate_url_frequency_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'usage_count' in df.columns:
            ...
            frequency_analysis['is_high_traffic'] = frequency_analysis['usage_count'] >= 5
            frequency_analysis['is_medium_traffic'] = (frequency_analysis['usage_count'] >= 2) & (frequency_analysis['usage_count'] < 5)
            frequency_analysis['is_low_traffic'] = frequency_analysis['usage_count'] == 1
    ...
    def generate_domain_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        domain_analysis.columns = ['Total URLs', 'Valid URLs', 'Total Usage', 'Avg Response Time']
        domain_analysis['Success Rate %'] = (domain_analysis['Valid URLs'] / domain_analysis['Total URLs'] * 100).round(1)
        domain_analysis = domain_analysis.sort_values('Total Usage', ascending=False)
        
        return domain_analysis
    
    def save_reports(self, summary_report: str, broken_links_df: pd.DataFrame, 
                    frequency_df: pd.DataFrame, domain_df: pd.DataFrame):
        ...
        self.reports_generated = [
            summary_file, broken_file if not broken_links_df.empty else None,
