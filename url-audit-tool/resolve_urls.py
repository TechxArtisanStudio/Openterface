...
from config import (
    OUTPUT_DIR, BASE_URL, SUPPORTED_LANGUAGES, CSV_COLUMNS
)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/resolve_urls.log'),
...
class URLResolver:
    ...
    def resolve_relative_url(self, url: str, language: str = 'en') -> str:
        ...
        # Check if URL already has a language prefix
        has_language_prefix = self.has_language_prefix(clean_url)
        
        # Handle different relative URL patterns
        if clean_url.startswith('/'):
            # Absolute path from root
            if has_language_prefix:
                # URL already has language prefix, don't add another one
                resolved = f"{BASE_URL}{clean_url}"
            elif language == 'en':
                # English is the default, no language prefix needed
                resolved = f"{BASE_URL}{clean_url}"
            else:
                # Add language prefix
                resolved = f"{BASE_URL}/{language}{clean_url}"
        else:
    ...
    def load_extracted_links(self, language: str = None) -> List[Dict]:
        ...
        if language:
            ...
            if csv_file.exists():
                ...
                logger.warning(f"No extracted links file found for language: {language}")
        else:
            # Load all languages
    ...
    def save_resolved_links(self, links: List[Dict]):
        ...
        for language, lang_links in links_by_language.items():
            ...
            logger.info(f"Saved {len(lang_links)} resolved links for {language} to {output_file}")
    ...
    def print_summary(self, links_by_language: Dict[str, List[Dict]]):
        ...
        for language, links in links_by_language.items():
            ...
            print(f"  {language}: {len(links)} links ({external_count} external, {relative_count} relative)")
