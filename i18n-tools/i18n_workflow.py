...
Usage:
  python i18n_workflow.py [command] [options]
...
class I18nWorkflow:
    ...
    def __init__(self, project_root: Optional[str] = None):
        ...
        if project_root:
            ...
            # If script is in i18n-tools/, go up one level to find project root
            script_dir = Path(__file__).parent
            if script_dir.name == "i18n-tools":
                self.project_root = script_dir.parent
            else:
                self.project_root = Path.cwd()
        
        self.i18n_tools_dir = self.project_root / "i18n-tools"
        self.docs_dir = self.project_root / "docs"
        self.mkdocs_config = self.project_root / "mkdocs.yml"
        self.i18n_config = self.i18n_tools_dir / "i18n.yml"
    ...
    def generate_config(self, force: bool = False) -> bool:
        """Generate i18n.yml from mkdocs.yml."""
        cmd = [
            sys.executable,
            str(self.i18n_tools_dir / "i18n_1_from_mkdocs.py"),
            "--mkdocs", str(self.mkdocs_config),
            "--output", str(self.i18n_config)
        ]
        
        if force:
            cmd.append("--force")
        
        return self.run_command(cmd, "Generating i18n config from mkdocs.yml")
    
    def run_audit(self, only_missing: bool = False, fail_on_missing: bool = False) -> bool:
        """Run translation coverage audit."""
        cmd = [
            sys.executable,
            str(self.i18n_tools_dir / "i18n_2_audit.py"),
            "--docs", str(self.docs_dir),
            "--config", str(self.i18n_config),
    ...
    def generate_prompts(self, overwrite: bool = False) -> bool:
        ...
        cmd = [
            sys.executable,
            str(self.i18n_tools_dir / "i18n_4_generate_prompts.py"),
            "--docs", str(self.docs_dir),
            "--config", str(self.i18n_config),
    ...
    def quick_refresh(self) -> bool:
        ...
        print("\n1️⃣ Generating i18n config...")
        if not self.generate_config(force=True):
            return False
        
        # Step 2: Run audit
        print("\n2️⃣ Running translation audit...")
        if not self.run_audit():
            return False
        
        # Step 3: Generate prompts
        print("\n3️⃣ Generating LLM prompts...")
    ...
    def show_status(self) -> None:
        ...
        files_status = {
            ...
            "i18n.yml": self.i18n_config.exists(),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="refresh",
        choices=["config", "audit", "prompts", "report", "refresh", "guides", "status", "cleanup"],
...
def main() -> int:
    ...
    if args.command == "config":
        success = workflow.generate_config(force=args.force)
