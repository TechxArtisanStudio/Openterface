 ...
 * 3. Add data-i18n-attr="attribute" to translate attributes (placeholder, value, aria-label, etc.)
 * 4. Add data-i18n-html="true" to allow HTML content (use cautiously)
 * 5. Place translation JSON in /assets/i18n-sites/filename.json
 * 
 * The handler runs automatically on page load and provides a global API.
 */
...
(function() {
    ...
    window.OpenterfaceI18n = {
        ...
        detectLanguage: function() {
            ...
            // Supported language codes (from i18n.yml)
        ...
        updateElement: function(element, translationData) {
            ...
            if (t[key]) {
                // Check if we should update attribute instead of textContent
                const attr = element.getAttribute('data-i18n-attr');
                const isHtml = element.getAttribute('data-i18n-html') === 'true';
                
                if (attr) {
                    // Update specific attribute (placeholder, value, aria-label, etc.)
                    element.setAttribute(attr, t[key]);
                } else if (isHtml) {
                    // Update innerHTML for rich content (use cautiously)
         ...
         */
        getTranslation: function(filename, key, fallback = '') {
            const translationData = this.translations[filename];
            if (!translationData) return fallback;
        ...
        init: function() {
            ...
            console.log(`i18n-handler: Initialized with language '${this.currentLanguage}'`);
    ...
    // Re-initialize on MkDocs navigation (for SPA-like behavior)
    document.addEventListener('DOMContentLoaded', function() {
        // Listen for MkDocs instant loading navigation events
