...
### **네트워크 독립성 및 초고속 응답**
내장 비디오 캡처 및 에뮬레이션된 키보드/마우스(HID) 입력을 사용한 안정적인 헤드리스 제어. 네트워크 연결이 필요하지 않습니다. 하드웨어 시작 시간이 1초 미만으로 워크플로우 지연 없이 즉각적인 문제 해결을 보장합니다.
...
- **4K 모드**: 실험적 기능으로 사용 가능*
- **압축**: YUV420, YUV444 및 MJPEG 지원

*참고: 4K 모드는 추가 열을 발생시킵니다. 장시간 작동 중 장치 표면이 매우 뜨거워질 수 있습니다
...
- **호스트 → 대상**: 에뮬레이션된 키 입력을 통해 텍스트 전송
- **대상 → 호스트**: OCR을 통해 대상 화면에서 호스트로 텍스트 복사 (macOS 전용)
...
!!! warning "텍스트 전송 제한 사항"
    ...
    - **OCR 기능**: 대상 → 호스트 텍스트 전송은 현재 macOS에서만 사용 가능합니다
...
### **블루투스 기능**
내장 블루투스 지원으로 네이티브 iPadOS 앱 기능을 활성화하여(곧 출시) KVM-GO를 iPad와 네이티브로 작동하는 몇 안 되는 KVM 중 하나로 만듭니다.
