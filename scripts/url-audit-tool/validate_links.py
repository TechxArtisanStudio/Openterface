...
import pandas as pd
import requests
from config import LOGS_DIR, OUTPUT_DIR, SUPPORTED_LANGUAGES, VALIDATION_CONFIG
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOGS_DIR / "validate_links.log"), logging.StreamHandler()],
)
...
class LinkValidator:
    ...
    def should_skip_validation(self, url: str) -> bool:
        ...
        try:
            # Skip protocol-specific URLs (mailto:, tel:, etc.)
        ...
        try:
            ...
            if domain in self.domain_last_request:
                ...
                if time_since_last < VALIDATION_CONFIG["delay_between_requests"]:
                    sleep_time = (
                        VALIDATION_CONFIG["delay_between_requests"] - time_since_last
                    )
    ...
    def detect_javascript_redirect(self, response) -> Optional[str]:
        """Detect JavaScript redirects in response content"""
        try:
            # Only check for JavaScript redirects in HTML responses
            ...
            for pattern in js_redirect_patterns:
                if pattern in content:
                    return f"JavaScript redirect detected: {pattern}"
    ...
    def validate_url(self, url: str) -> Dict:
        ...
        if self.should_skip_validation(url):
            ...
            if self.is_protocol_specific_url(url):
                ...
                result["error_message"] = f"Skipped ({protocol} protocol)"
            elif "{{" in url and "}}" in url:
                result["error_message"] = "Skipped (template variable)"
            else:
                result["error_message"] = "Skipped (local domain)"
        ...
        try:
            ...
            response = self.session.head(
                url, timeout=VALIDATION_CONFIG["timeout"], allow_redirects=True
            )
            ...
            # Check for JavaScript redirects on error pages (404, etc.)
            if not result["is_valid"] and response.status_code == 404:
                # For 404 pages, make a GET request to check for JavaScript redirects
                try:
                    get_response = self.session.get(
                        url,
                        timeout=VALIDATION_CONFIG["timeout"],
                        allow_redirects=False,  # Don't follow redirects for content analysis
                    )
                    js_redirect = self.detect_javascript_redirect(get_response)
                    if js_redirect:
                        result["js_redirect_detected"] = js_redirect
                        result["error_message"] = (
                            f"HTTP {response.status_code} with JavaScript redirect"
                        )
                    else:
    ...
    def load_resolved_links(self, language: str = None) -> List[Dict]:
        ...
        if language:
            ...
            if csv_file.exists():
                ...
                logger.info(
                    f"Loaded {len(links)} resolved links for language: {language}"
                )
            else:
            ...
            for lang in SUPPORTED_LANGUAGES.keys():
                ...
                if csv_file.exists():
                    ...
                    logger.info(
                        f"Loaded {len(lang_links)} resolved links for language: {lang}"
                    )
            ...
            validation_result.update(
                {
                    ...
                    "file_locations": "; ".join(
                        [
                            f"{info['file_path']}:{info['line_number']}"
    ...
    def save_validation_results(self, results: List[Dict]):
        ...
        for language, lang_results in results_by_language.items():
            ...
            logger.info(
                f"Saved {len(lang_results)} validation results for {language} to {output_file}"
            )
    ...
    def print_summary(self, results: List[Dict]):
        ...
        if self.validation_stats["total_validated"] > 0:
            success_rate = (
                self.validation_stats["valid_links"]
                / self.validation_stats["total_validated"]
            ) * 100
...
def main():
    ...
    print(
        f"Note: This may take a while due to rate limiting ({VALIDATION_CONFIG['delay_between_requests']}s between requests)"
    )
