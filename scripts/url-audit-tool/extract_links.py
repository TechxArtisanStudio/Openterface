...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOGS_DIR / "extract_links.log"), logging.StreamHandler()],
)
...
class LinkExtractor:
    ...
    def extract_html_links(
        ...
        for line_num, line in enumerate(lines, 1):
            # Skip lines with Jinja2 template variables
            ...
            # Skip if line contains Jinja2 template variables
            if "{{" not in line or "}}" not in line:
                # HTML img src
                for match in re.finditer(
                    LINK_PATTERNS["html_img_src"], line, re.IGNORECASE
                ):
                    src = match.group(1)
                    # Skip if src contains template variables
                ...
                # HTML href
                for match in re.finditer(
                    LINK_PATTERNS["html_href"], line, re.IGNORECASE
                ):
                    href = match.group(1)
                    # Skip if href contains template variables
    ...
    def save_results(self, links: List[Dict]):
        ...
        for language, lang_links in links_by_language.items():
            ...
            logger.info(
                f"Saved {len(lang_links)} links for {language} to {output_file}"
            )
    ...
    def print_summary(self, links_by_language: Dict[str, List[Dict]]):
        ...
        print(
            f"Total links extracted: {sum(len(links) for links in links_by_language.values())}"
        )
        print("\nBy Language:")
        ...
        for language, links in links_by_language.items():
            ...
            print(
                f"  {language}: {len(links)} links ({external_count} external, {image_count} images)"
            )
